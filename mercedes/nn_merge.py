...
def the_metric(y_pred, y):
    y_true = y
    return 'my_r2', r2_score(np.array(y_true), np.array(y_pred))


try:
    t_train_int, t_train_str, t_test_int, t_test_str,label_train,label_val = pk.load(gzip.open('data/train_test_data.zip', 'rb'))
    ...
    train_str_data = train[d_t_str_keys]
    test_int_data = test[d_int_keys]
    test_str_data = test[d_t_str_keys]
    ...
    t_train_int = np.array(train_int_data)
    t_test_int = np.array(test_int_data)
    t_train_str = np.array(train_str_data)
    t_test_str = np.array(test_str_data)
    ...
    try:
        pk.dump([t_train_int, t_train_str, t_test_int, t_test_str,label_train,label_val], gzip.open('data/train_test_data.zip', 'wb'), -1)
        print 'Compressed pickling done'
    except Exception as e:
        print e, 'pickling error'
        pass
model = ma.fcnn(t_train_int.shape[1], t_train_str.shape[1])

t_board = keras.callbacks.TensorBoard(log_dir='./logs', histogram_freq=0, write_graph=True, write_images=False)
checkpoint = ModelCheckpoint('weights/best.model', monitor='val_r2_keras', mode='max', save_best_only=True, save_weights_only=False, verbose=2)

train_size = int(0.90*len(t_train_int))
history = model.fit([t_train_int[:train_size],t_train_str[:train_size]], label_train, validation_data=([t_train_int[train_size:], t_train_str[train_size:]], label_val), nb_epoch=300, batch_size=8, verbose=1, callbacks=[checkpoint])

#model_best = ma.fcnn(t_train.shape[1])
#model_best.load_weights('weights/best.weights')
model_best = load_model('weights/best.model', custom_objects={'r2_keras': ma.r2_keras})
#result = model.predict(t_train[train_size:])
#
#r2_score = the_metric(result, label_val)
y_pred_best = model_best.predict([t_test_int, t_test_str])
y_pred = model.predict([t_test_int, t_test_str])
